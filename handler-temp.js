...
module.exports.requestresponse = (event, context, callback) => {
  ...
  if(event.httpMethod === "POST" && event.body){
    ...
    if(Number(parseurl.Status) === 1) {
      ...
      // Chain with catch
      facebookcodehookreturnresponse(parseurl.RefNo)
      .then((res) => {
        ...
        // return lexruntime.postText(lexParams, (err, data) => {
        //     if (err) {
        //       console.log(err);
        ...
        return new Promise((resolve, reject) => {
          lexruntime.postText(lexParams, (err, data) => {
            if (err) {
                ...
                console.log('lexruntimepostText response: ', data);

                return axios.post(messengerEndpoint, msParams)
                  .then((res) => {
                    console.log('msgrendpoint post response: ', res);
                    resolve(res);
                  })
                  .catch((err) => {
                    console.log('msgrendpoint post error: ', err);
                    reject(err);
                  });
            } // else
          }); // lexruntime.postText
      ...
      .then((res) => {
        console.log('msgrendpoint post response outside: ', res);
      })
      .catch((err) => {
